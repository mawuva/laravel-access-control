...
trait ResourceDataManager
{
    ...
    public function validateAndGetResourceById(int|string $id, string $resource = null, $inTrashed = false)
    {
        ...
        if (is_null($data)) {
            throw new Exception(trans('accontrol::messages.resource.not_found'), Response::HTTP_NOT_FOUND);
        }
    ...
    public function validateSlug(string $slug, string $resource = null, int|string $id = null)
    {
        ...
        if ($data !== null) {
            if (is_null($id)) {
                throw new Exception(
                    trans('accontrol::messages.attribute_already_exists', [
                        'attribute' => trans('accontrol::entity.attributes.slug')
    ...
    {
        $model = config('accontrol.'.$this->resolveEntity($entity).'.model');
